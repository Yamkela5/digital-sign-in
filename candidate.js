...
function initializeApp() {
  // Load user profile from sessionStorage
  const profileLoaded = loadUserProfileFromSession();
  
  // If no profile in session, check if we should redirect to login
  if (!profileLoaded) {
    console.warn("No user profile found in session storage");
    // Uncomment the following line to enforce login
    // window.location.href = '/Login-page.html';
  }
